...
Every source read is pinned to one exact commit, and connecting a repository is all that takes. When a GitHub deployment is on record, the commit read is the one from the newest deployment at or before the end of the investigated window, whether it shipped last night or last month. When there is no deployment on record, the commit read is the newest one on your default branch. Either way the receipt names a full immutable SHA you can re-open later, and Flusterduck resolves abbreviated SHAs through GitHub before reading.

One thing depends on which of those two it was, and it is the riskiest claim in the system. Finding something in your source supports a diagnosis whichever commit it was read at. Failing to find something only means anything if the commit searched is what your visitors actually ran, and only a GitHub deployment establishes that. Read at a commit, an absence is reported as inconclusive rather than allowed to retract an issue somebody hit.
